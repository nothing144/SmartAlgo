...
def test_gemini_with_image():
    ...
    try:
        response = requests.post("https://neteval-debug.preview.emergentagent.com/api/submissions", 
        ...
        if response.status_code == 200:
            ...
            for i in range(10):
                time.sleep(2)
                detail_response = requests.get(f"https://neteval-debug.preview.emergentagent.com/api/submissions/{submission_id}")
